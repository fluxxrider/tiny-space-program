// Playtest "lune" phase 9: Verda in Lune's sky, framed properly (camera low, looking up at Verda) at the flat landing
...
export default async function (page, tools) {
  ...
  for (let k = 0; k < 4; k++) {
    ...
    await evalJS(`(TSP.flightScene.camera.setState({ mode: 'auto', yaw: ${g.yaw}, pitch: -0.3, distance: 8 }), true)`);
